...
// This map has all users connected linked as userid -> {socket_id : the.socket.id} , can be linked directly.
...
function onEachUserConnection(socket) {
	...
	// All events that can happen with this socket.
	onMessage(socket);
	checkOnline(socket);
...
function insertIntoPending(userId, message) async {
	const Messages = db.collection('Messages');
	await Messages.insertOne({message}, (err, insertedMsg) {
		if(err){
			// What to do if object not inserted ?
			print(err);
			return;
		}
		const Users = db.collection("Users");
		var xyz = await Users.update(
			{_id: userId}, 
			{ $push: { PendingMsgs: insertedMsg._id} }
		);
	});
}

function retreiveFromPending(userId) async {
	const Users = db.collection("Users");
	var pendingMsgs = await Users.find({_id:userId}, {_id:0,pendingMsgs:1})..toArray();
}
...
function singleChatHandler(socket, chat_message) {
	...
	if (!userOnline) {
		...
		// So Now we push this message into the database.
	...
	// this will be used for read recepients.
	chat_message.message_sent_status = STATUS_MESSAGE_SENT;
	chat_message.to_user_online_status = false;
	sendBackToClient(socket, SUB_EVENT_MESSAGE_FROM_SERVER, chat_message);
